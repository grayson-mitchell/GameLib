...
// colour variants below. `public/icon-tray-source.png` and `public/icon-light.png` were found to be
...
///
/// TRANSITIONAL: this points at the MASTER ARTWORK, which is what the "dark" slot has always
/// displayed -- the rename to `icon-tray-source.png` only made that explicit and changed no
/// bytes. The rename exists so the generator cannot overwrite its own input once it starts
/// emitting a real Windows/Linux pair; at that point this should point at the GENERATED dark
/// variant, not at the source.
const TRAY_ICON_DARK: &[u8] = include_bytes!("../../public/icon-tray-source.png");
