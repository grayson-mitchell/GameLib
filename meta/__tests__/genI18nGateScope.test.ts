 ...
 *
 * 2026-09-02 (quick `260902-wbd`, porting the login-background setting from the
 * `wip/login-background-260815` stash): scope and fork-touched each grow by exactly
 * ONE file, to 164 and 209 respectively. `Settings/components/LoginBackground.tsx` is
 * a new fork-authored settings component, so it enters BOTH lists in the same sorted
 * slot -- unscanned stays UNCHANGED at 45 because the file never appears in the
 * `forkTouched - committedScope` difference. This is a widening of the BLOCKING
 * hardcoded-string gate by one file, measured with
 ...
 *
 * 2026-09-05 (Phase 40 verification gap GAP-C, the ADD half): fork-touched
 * 208 -> 213. Plan 01's re-derive above was correct AND INCOMPLETE AS A PHASE
 * RECORD: it ran in wave 1 and moved the pin DOWN for two files it deleted,
 * but waves 3-6 then CREATED five new string-carrying frontend files and
 * nobody re-ran this gate, so the artifact rotted in the opposite direction
 * and `A-17 ANTI-ROT` went red. The lesson is about ORDERING, not arithmetic:
 * a scope pin re-derived before the phase stops creating files measures a
 * world that no longer exists by the time the phase ends. Added:
 * `components/UI/NavShell/StoreEmbedSuppressionContext.tsx` (40-06),
 * `components/UI/StoreEmbedControls/index.tsx` (40-07),
 * `screens/WebView/components/StoreEmbedPlaceholder.tsx` (40-06),
 * `screens/WebView/storeEmbedOrigins.ts` (40-09) and
 * `screens/WebView/useStoreEmbedHost.ts` (40-08).
 *
 * Derived, not assumed: `buildScopeSnapshot()` was called against the real
 * merge-base diff and its output compared to the committed artifact, giving
 * exactly these five additions and ZERO removals. That import was made with
 * `JEST_WORKER_ID` set, because `genI18nGateScope.ts` runs `main()` at import
 * time otherwise and would have REWRITTEN the very artifact being measured --
...
describe('--rewrite-scope guard', () => {
   ...
   * landed).
   *
   * 2026-09-05 (quick `260905-d33`): scope 169 -> 171, fork-touched 213 -> 215,
   * unscanned debt UNCHANGED at 44. Two new files, both added to BOTH artifacts
   * (which is why the debt does not move): `components/UI/AboutDialog/index.tsx`
   * and its `AboutDialogHost.tsx`, the in-app modal that replaced the About OS
   * window. Hand-edited surgically, NOT regenerated, per the 260901-w9e /
   * 260902-qs4 precedent above. Putting AboutDialog in SCOPE rather than debt
   ...
   *
   * 2026-09-21 (closing the library-searchbar-mouse-dead debug session / the
   * 2026-08-30 Library SearchBar todo): fork-touched 214 -> 213, unscanned
   * debt 42 -> 41, scope UNCHANGED at 172. `SearchBar/searchProbe.ts`
   * DELETED outright -- given the live-drive findings, the operator chose to
   * remove the whole `.autoComplete` suggestions overlay from
   * `LibrarySearchBar`/`SearchBar` rather than fix its hover defect (see
   * `.planning/todos/completed/2026-08-30-library-search-bar-suggestions-
   * are-mouse-dead-until-a-tab-press.md`). Same precedent as `260901-w9e`: a
   * deleted file leaves both artifacts. Removed from `DECLARED_UNSCANNED_DEBT`
   * above and from `meta/i18nForkTouchedFiles.json`, `generatedAt` held
   * constant per this file's own precedent. Hand-edited surgically, NOT
   * regenerated via `pnpm gen-i18n-gate-scope`.
   *
