...
## Live gate — RUN 2026-08-17 (NZST), branch `fix/steam-native-install-stability`: **PASS**

Electron build (`pnpm start`) at HEAD `83a9de3e6`, clean tree. Installed HUMANKIND
(`1124300`) — the same title as the original observation — and let the 8-minute watchdog
trip naturally. No code changes, no network manipulation: `install()` awaits the whole
download, so any install still running at 8:00 trips it.

All six recipe steps closed:

1. **Failure produced** — `(09:24:09) Installation of 1124300 failed with: install did not
   settle — connection may be stale`.
2. **Abort line present, same second** — `(09:24:09) [DownloadManager]: Aborting in-flight
   download for 1124300 after terminal install failure`, followed immediately by
   `(09:24:09) [Steam]: SteamGame: aborting in-flight native depot download for appId 1124300`.
3. *(ordering note below)*
4. **PROOF BY ABSENCE — the chunk loop stopped.** Stats had been landing every ~17s right up
   to the trip (`@377s @394s @410s @427s @444s @460s @476s`). After the abort: one final
   `@477s` line 1s later with `downSpeedMiBs=0.00`, then **zero** further
   `[Timing] chunk-stream stats` lines across a 180s observation window. Contrast the pre-fix
   run recorded above: ~5 more minutes of chunks through `@696s`.
5. **On-disk file count froze** at **3,457** — identical at t+60s, t+120s and t+180s.
6. **Immediate retry starts a FRESH run.** A second Install click at 09:31:41 produced a full
   new pipeline rather than joining the tearing-down run: `runNativeDepotDownload:
   ensureSteamClientReady` (09:31:59) → `resolveSteamInstallTarget` (09:32:00) →
   `buildDepotPlan total 6181ms` (09:32:06) → `downloadDepotFiles: first bytes written`
   (09:32:19). `nativeInstallsInFlight` is genuinely released on the failure path.

**Bonus bullet confirmed live:** `downloadSteamDepots ... (status=cancelled)` at 09:24:10, so
`runNativeDepotDownload`'s cancelled branch runs and `markSteamInstallIncomplete` fires.

Evidence log: `<scratchpad>/abort-gate/RUN-20260817-humankind-watchdog.log`.

### Gate-harness correction (worth carrying forward)

The harness's first run reported **FAIL** — a false negative. The abort lines are emitted from
`installQueueElement`'s `finally`, which runs **before** the caller in `downloadqueue.ts` logs
`Installation of X failed with:`. They therefore land **above** the failure line in the log,
and a window that scans only forward from the failure line finds nothing.

The hand-authored "expected-good" specimen used to validate the harness had invented the
opposite ordering, so it validated a log shape production never emits — the same
`test-must-exercise-production-call-shape` trap. Fixed by matching abort lines across the whole
log and bounding by **timestamp proximity** instead of line order, then re-validating against
both a known-bad specimen (still FAILs) and the real capture (PASSes).

### Deferred bullet — status changed

"Partial bytes left by an aborted run are either cleaned up or made reconcilable" remains open
in `2026-08-16-aborted-depot-residue-has-no-acf.md`, **but that todo's premise needs
re-checking**: this run DID leave an `appmanifest_1124300.acf` (StateFlags `1026`, written
09:24). The "no `.acf` is ever written" premise may not hold for the watchdog-abort path.

### New defect found by this gate

The watchdog trip was not a stale connection — it was an 8-minute duration ceiling on every
native Steam install. Filed as
`2026-08-16-eight-minute-install-watchdog-makes-long-native-steam-instal.md` (**major**). It
also retires the "Suspected trigger" inference above: the library-sync canary race was a red
herring.
