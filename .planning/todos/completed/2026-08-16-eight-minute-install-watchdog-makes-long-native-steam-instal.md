...
Operator ran a full native HUMANKIND (appId `1124300`) install and reported it complete. Verified
against on-disk state rather than log lines — see caveat below:

```
appmanifest_1124300.acf (mtime 2026-08-18 13:53:42, i.e. today):
  StateFlags       "4"
  BytesToDownload  "37592580261"
  BytesDownloaded  "37592580261"     <- equals BytesToDownload: fully settled, not a partial/aborted write
  InstalledDepots: 1124302 (37342725351 bytes), 1124303 (249854910 bytes) -- both present
```

`du -sh` on the installed directory independently confirms ~35G on disk, matching `SizeOnDisk`.
At the ~60s/GB rate this debug arc established for Mac-native decode
(`.planning/debug/humankind-depot-full-stall.md`, 260817-sw8's Planetfall measurement), a 35GB
install needing ~35 minutes could only complete without tripping if the no-progress watchdog is
in fact scoped correctly — the original defect (an 8-minute **total-duration** ceiling) would have
killed this run at 8m00s regardless of health, and the abort path (`260816-vgc`) would have left
no complete `.acf` behind. A fully-settled manifest with `BytesDownloaded == BytesToDownload` is
evidence the download was never aborted.

**Caveat — no quoted log lines, unlike every other closure in this debug arc.** The install's own
`gamelib.log` output was not captured before this session started: the ACF's mtime (13:53:42)
predates both `gamelib.log.old` (starts 14:27:56) and the current `gamelib.log` (starts 15:03:13),
and grepping `1124300` across every rotated log file in `~/Library/Logs/GameLib/` (including
`.old`, `.run3`, and the dated gate-archive copies) returns zero hits — the run's log window was
already rotated out by the time this closure was written. This also means **native-decode
engagement for this specific run is unconfirmed** (a separate question from this todo — see
Phase 23.1's own blocking checkpoint, plan `23.1-05` Task 2, which needs its own quoted evidence
and is not satisfied by this closure).

Closing on the strength of the on-disk ACF state, which directly matches this todo's own stated
Verification criterion ("reaches 100% and writes its `appmanifest_*.acf` rather than tripping at
8m00s") — but noting the process gap (capture `gamelib.log` *before* a live-gate run next time, or
copy it off immediately after) so a future closure isn't working from filesystem inference alone.
