...
- [ ] 34.6-18-PLAN.md — gap wave 1: replace `moveInstall`/`importGame`'s `defaultInstallPath` containment (which rejected the cross-drive move / out-of-tree import each feature exists for, and was circular because the root is renderer-writable via `setSetting`) with `rendererPathGuard.assertPlausibleAbsolutePath`; cross-root ACCEPT RED-proven first; both `T-34.5-C6-49-03` header comments re-dispositioned in the same commit
- [ ] 34.6-19-PLAN.md — gap wave 2: make a rejection USER-VISIBLE (three `gamelib:installFlows.*` strings in `public/locales/en/gamelib.json`, the `gamelib` namespace actually loaded at both i18next init sites, `showDialogBoxModalAuto` on both rejection paths) without regressing T-34.6-31/T-34.6-32
- [ ] 34.6-20-PLAN.md — gap wave 3: REQUIREMENTS.md corrections (REQ-34.6-05's false "REQUIREMENT NOW FULLY COMPLETE" removed and left `[ ]` pending the live re-drive; REQ-34.6-10/-11/-12/-14 stale checkboxes flipped to `[x]`), `34.6-PORTED-CHANNELS.md` row corrections, `34.6-VALIDATION.md` map extension, CR-01 todo closed with `resolves_phase: 34.6`
