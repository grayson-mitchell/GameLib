...
### Phase 34.6: Tauri IPC re-plumb slice 9 — EOS overlay, SteamGridDB artwork, winetricks + the Epic/save-sync verification inherited from 34.7 (INSERTED) — ✅ COMPLETE 2026-08-26
...
**Plans:** 21 plans — **21/21 executed on disk**; 34.6-19..21 are the remainder of the VERIFICATION-driven gap cycle (gap cycle 2), authored 2026-08-25 and executed 2026-08-26. The 14 planned plans (34.6-01..14) plus a three-plan gap cycle (34.6-15, 34.6-16, 34.6-17) authored 2026-08-25 (`540d326b7`) because plan 34.6-12's Task 3 forbids proceeding to 34.6-13/14 on a FAIL verdict. **The phase closes FAIL 7/9** — see `34.6-LIVE-GATE.md` and the 34.6-17 row below.
...
- [x] 34.6-21-PLAN.md — gap wave 4 (NOT autonomous): APPEND a `## GAP CYCLE 2` section to the closed `34.6-LIVE-GATE.md` (never re-score the FAIL 7/9) with three contract-first items driving the never-tested out-of-root case in both directions, the macOS `openrsync` confound made distinguishable by construction; REQ-34.6-05 then closes or re-blocks on the MEASURED result — DONE 2026-08-26 (see `34.6-21-SUMMARY.md`)
...
**Plans:** 19 wave plans (13 waves) + 10 gap-closure plans `35-20`..`35-29` (3 waves), 29 total — **EXECUTING, 18/19 wave plans (waves 1-12 COMPLETE as of 35-18, 2026-08-30 — 35-18 removed `electron` from `package.json`/the esbuild alias, mechanized D-03 as `meta/__tests__/electronAbsence.test.ts`, and shipped `35-RELEASE-NOTES.md`; only 35-19's wave-13 blocking packaged macOS arm64 live gate remains. **Gap-closure wave 1: `35-20` COMPLETE (2026-08-30)** — closed live-gate criteria 6/10/14 (`D-35-19-05`/`-06`/`-09`) via a shared `dispatchSteamLaunch` helper used by both the sidecar `launch` handler and the `gamelib://` deep-link path, `RUNNERS` widened to include `steam` (excluding `zoom`), and the `installed.json` watcher now sends `refreshLibrary` to the renderer after a successful debounced refresh. **`35-21` COMPLETE (2026-08-30)** — closed BOTH `35-REVIEW.md` criticals CR-01 (`open_external` now enforces an explicit five-scheme allow-list — `https`/`http`/`mailto`/`tel`/`steam` — inside the Rust command itself, ahead of `app.opener().open_url`, since Rust-side plugin calls bypass the capability system entirely) and CR-02 (`frontendReady`'s ported boot-time `initQueue(true)` auto-resume now guarded by a module-scoped `frontendReadyBootWorkDone` flag, set before scheduling so same-tick double delivery cannot both observe `false`; the registration itself stays `ipcMain.on` since the Snap-warning dialog and log lines ahead of the guard are allowed to repeat per T-35-107); both RED-proofs recorded verbatim in `35-21-SUMMARY.md`, commits `94e5f88ac`/`fb12d4261`. **`35-22` COMPLETE (2026-08-30)** — closed CR-03 (`window.platform` gains a reachable `'win32'` arm, derived from two independent signals via a new `isWindowsWebview()` single-sourced in `platformDetect.ts`; the shipped NSIS build previously fell through to `'linux'` on all 72 `platform === '...'` call sites under `src/frontend`) and the renderer half of CR-04 (`Runner/index.tsx`'s `handleLogout` catch, previously a bare `console.error` invisible under Tauri, now calls `window.api.logError(...)` and raises a `showDialogModal` ERROR dialog using two new `gamelib.json` keys); `src/preload/index.ts` deleted as dead code (zero real importers, re-verified by grep); both RED-proofs recorded verbatim in `35-22-SUMMARY.md`, commits `f20b90af6`/`bbbdb92fd`/`635151971`. **`35-23` COMPLETE (2026-08-30)** closed CR-04's backend half and REQ-35-07's remaining code gap: `legendary/user.ts`'s `clearEpicCookies` gained a per-host before/after cookie census (ported from `humble/user.ts`'s pattern) and a three-case jar-liveness fatality rule replacing the bare `total === 0` check — a host proven populated with a zero delta is FATAL regardless of the overall summed total, RED-proven against a naive `total===0`-only implementation. `EPIC_COOKIE_HOSTS`/`FATAL_WIPE_STEP` byte-identical; 9 new tests in `epicCookieCensus.test.ts`; commits `acf854233`/`b5ed7df03`. `D-35-19-15` NOT closed — live proof deferred to plan `35-29`'s criterion-21 re-run. **`35-24` COMPLETE (2026-08-30)** — closed `35-VERIFICATION.md` gap 4 (A-17 ANTI-ROT): re-measured the fork-touch drift from scratch rather than trusting the plan's own stale 199/163 baseline, found the real delta was 199 -> 205 (six Phase 35 files), regenerated `meta/i18nForkTouchedFiles.json` via `pnpm gen-i18n-gate-scope` and re-baselined every dependent `--rewrite-scope guard` fixture count/title plus the `DECLARED_UNSCANNED_DEBT` list in the SAME commit — the naive fix (regenerate only) is measured on this repo to cascade 1 failure to 5; commit `ee86b3442`. `meta/i18nGateScope.json` (hand-curated) verified untouched. 9/10 gap-closure plans executed on disk (`35-25` also COMPLETE, 2026-08-30 — closed `REQ-35-16`'s winetricks clause; see its own bullet below; `35-26` also COMPLETE, 2026-08-30 — closed REQ-35-17's EOS half, moving both confirmations to `showDialogModal` fail-closed on `=== true`; a first live-gate attempt FAILED on bare Install/Update buttons, fixed and re-verified PASS on attempt 2; see its own bullet below; `35-27` also COMPLETE, 2026-08-31 — closed the code-level cause of criterion 16 (`D-35-08-02`) via `classifySleepAssertionKind`/`reconcileSleepAssertionCalls` in `GlobalState.tsx` (option-a), both exposing faces RED-proven verbatim; does not itself close REQ-35-20, live re-measure deferred to `35-29`; see its own bullet below; `35-28` also COMPLETE, 2026-08-31 — records-hygiene reconciliation of the Phase 35 REQUIREMENTS.md status table and the `completed_plans` counter; see its own bullet below). This row was stale at 12/19 through waves 1-5 for several sessions; 35-09 through 35-12, 35-13 through 35-16, 35-17, and 35-18 all landed since and are corrected above). `35-09` RE-PLANNED — mechanism banned in source, see `D-35-09-01`; superseded plan retained, cookie clear landed. `35-13` unblocked and complete. `35-10` PARTIAL — Task 1 shipped, Task 2 blocked. `35-07`/`35-08` code-complete but NOT discharged. `D-35-10-01` is TIME-CRITICAL: `uncaughtException` has no sidecar equivalent and dies at wave 8.** `35-01` preflight: OQ-1 measured `AGREES` on a real packaged SEA run (`main=true worker=true`), OQ-4 `NEEDS-HUMAN-SMOKE`, PD-B corrected CONTEXT's 22-export framing to 19 live + 3 internal-only, and CENSUS-E2E confirmed the `CI=e2e` harness is Electron-only with no Tauri equivalent (a correction owed to D-19). `35-02` D-18 A/B re-test: blocking human gate DRIVEN across both shells, 7/7 items scored — 1 `BOTH`, 2 `NOT ATTEMPTED`, 3 **`TAURI-ONLY` and the ONLY item meeting its BLOCKS-D-16 pre-call (carried to 35-19)**, 4 `TAURI-ONLY`, 5 `NOT ATTEMPTED`, 6 `BOTH` with (b) TAURI-ONLY INVERTED (the path-rejection dialog exists only under Tauri — the one surface where Tauri beats Electron), 7 `TAURI-ONLY` for an INCOMPLETE clear (the recorded false-report symptom is FIXED and `EPIC_SESSION_AP` is measured ABSENT, falsifying that item's stated blocking mechanism; six live epicgames.com cookies survive). Item 1 was corrected `NEITHER`->`BOTH` and item 2 `NEITHER`->`NOT ATTEMPTED` after checking each item's nominated evidence — the stale-`nonAvailableGames` probe fired on ALL SIX uninstalls across BOTH shells (shared code, so it SURVIVES the cutover), and `installed.json` had not been written in four days so item 2 tested nothing. Addendum A-1 filed as its own todo: `moveInstall` fails outright with `rsync: unrecognized option '--no-human-readable'`, shared backend code that likewise survives 35-14. **Wave 2 (`35-03`, `35-06`):** `35-03` migrated the renderer off `electron-vite` onto plain Vite and gave `tauri:dev` real HMR via `devUrl` — and found that `electron-vite` INJECTS config the visible `electron.vite.config.ts` never shows, load-bearingly `base: './'`; a faithful lift of the visible block would have emitted `/assets/...` and 404'd every asset in a packaged bundle, the exact `R-34.5-G1-PKG` shape. Verified by building an electron-vite baseline and diffing: `index.html` byte-identical, all four referenced assets sha256-identical, 12 symlinks restored. **`vite` is NOT a direct dependency** (hoisted peer of `electron-vite`), contradicting `35-RESEARCH.md`'s T-35-SC — filed as `D-35-03-02`, a BLOCKING input for 35-14, whose own verification step is `pnpm exec vite build`. `35-06` extended the Tauri tray (recent games, About, all four settings) and drove its blocking live gate: steps 1-5 PASS, step 6 **NOT EXECUTABLE** (icons are `include_bytes!`, so no runtime action reaches the failure arms). **The gate found THREE defects in code tasks 1-2 had called correct** — `exitToTray` read from a startup snapshot whose rationale named the other two settings and not it; `open_devtools()` forced the window visible, defeating `startInTray` and making step 5c VACUOUS; and `TRAY_SETTINGS` had two `OnceLock` initialisers where whichever ran first won permanently. All three fixed (`caa84b46b`, `918d2afb3`, `c95a1efcb`), each gate proven RED against the pre-fix shape. The About window now has its first caller under Tauri, closing that folded todo. Operator-initiated mid-gate: `RecentGame` was LOSSY — `addRecentGame(game: GameInfo)` discarded `game.runner`, so `trayResolveRunner` was reconstructing by probe a value thrown away one line earlier; runner now persisted, probe demoted to legacy fallback. **Wave 3 (`35-04`, `35-05`, `35-11`):** `35-04` closed **`R-34.5-G1-PKG` on BOTH halves against a real packaged DMG** — `Contents/Resources/build/` now carries 147/147 locale files across 49 languages where the 2026-08-22 probe found only `_up_` and `icon.icns`; `Contents/Resources/public` verified ABSENT, so the operator's packaged-app confirmation (real English, working language switch) can only come from `build/`. `app.isPackaged` is now ONE `node:sea` derivation with three callers. **`G-34.2-UAT-02`'s blocker is discharged but its NAMED evidence is NOT** — the repair-notification observation was never re-made, and `34.2-HUMAN-UAT.md` now separates RESOLVED from STILL OUTSTANDING and routes it to 35-19 by name, because that document explicitly refused the substitution in advance. `35-05` swapped `electron-store` for a first-party `conf@10.2.0` shim — **the plan's own prescribed method was measured to cause total silent data loss** (`conf` has no `name` option and resolves a relative `cwd` against `process.cwd()`, so all 24 cache files would have collapsed onto one `config.json` inside the repo), and true scope was ~48 sites not 9. `electron-store` was re-added as a DEVDEPENDENCY (`D-35-05-05`, owner 35-16) because one deferred consumer remains and a clean install would otherwise break `pnpm start` at boot. `35-11` fixed the error dialog's fixed `height: 25em`, ported the boot download-queue auto-resume, and closed both SEAM items — **the plan's premise was inverted on both dialogs** (path-rejection is app-styled-but-oversized; EOS is native-but-correctly-sized) and **REQ-35-17 is satisfied on path-rejection ONLY**, EOS blocked by `D-35-11-01`. Live gate also surfaced a shared-frontend defect that survives 35-14: the Pause/Cancel button opens the install modal mid-download for non-Steam games (`MainButton.tsx:305` guards on `runner !== 'steam'` but not `!is.installing`, while the same component's label DOES consult it).
...
- [x] 35-01-PLAN.md — Pre-flight probes and censuses: the five research open questions + two pattern-mapper decisions (wave 1)
- [x] 35-02-PLAN.md — D-18 A/B re-test of 7 parked bugs and folded todos under both shells (wave 1)
- [x] 35-03-PLAN.md — Vite migration off electron-vite + HMR dev loop with a packaged-evidence path (wave 2)
- [x] 35-04-PLAN.md — `R-34.5-G1-PKG` both halves + `isPackaged` collapsed to one derivation (wave 3)
- [x] 35-05-PLAN.md — `electron-store` -> `conf` (wave 3) — **re-scoped 9 -> ~48 sites during execution; the plan's prescribed method would have caused total silent data loss. `electron-store` RETAINED as a devDependency until 35-16 (`D-35-05-05`)**
- [x] 35-06-PLAN.md — Tray extension, reachable About window, `TraySettings` affordance audit (wave 2)
- [x] 35-07-PLAN.md — `gamelib://` OS registration (macOS+Linux, `option-c`) + `openDialog` long-running channel (wave 4) — **code complete; Task 4 packaged-build human-verify still owed**
- [x] 35-08-PLAN.md — Real wake-lock assertions replacing the `powerSaveBlocker` no-op (wave 5) — **code complete; Task 3 live human-verify still owed. 2 acceptance criteria UNMET BY DESIGN (`D-35-08-01`) — score SUPERSEDED, do NOT add `generate_handler!` entries**
- [x] 35-09-PLAN.md — **RE-PLANNED** (superseded plan retained). Epic cookie clear widened to 5 Epic-owned apex domains, Rust+TS together; a zero-total clear now FAILS the logout. Code complete; the 34.6 Step 8 re-run (BOTH clauses) still owed. `D-35-09-02`: a failed clear is invisible to the user (wave 6)
- [~] 35-10-PLAN.md — `installed.json` watcher port (SHIPPED) + `winetricksInstall` (BLOCKED — all 3 hypotheses refuted; real cause is renderer hit-testing, unnamed after 5 drives) (wave 7)
- [x] 35-11-PLAN.md — Dialog restyles + SEAM Phase 31 D-02 / Phase 33 D-04 convergence (wave 3)
- [x] 35-12-PLAN.md — Flatpak/Flathub deletion + AppImage-only target pin (wave 4)
- [x] 35-13-PLAN.md — `backend/platform` created and unconsumed, with first-party electron types (wave 7) — complete `bcf257600`
- [x] 35-14-PLAN.md — **POINT OF NO RETURN**: tag, then delete the Electron entry points (wave 8) — complete `5ee4d8302`
- [x] 35-15-PLAN.md — The 67-file mechanical import rewrite + reach-ledger baseline (wave 9) — complete `5e8ad98e3`
- [x] 35-16-PLAN.md — Preload collapse: secret-policy unification + the two grep-invisible import forms (wave 10) — complete 2026-08-29, 4 task/fix commits (`8dafd8d91`, `3f31b37fd`, `e975bb456`, `5dfd07e07`)
- [x] 35-17-PLAN.md — `isTauri()` collapse across 28 files / 140 references (wave 11) — complete 2026-08-29, definition deleted from `src/preload/tauriTransport.ts` + `meta/__tests__/isTauriRemoved.test.ts` mutation-proven static absence gate added, 8 task commits (`a4f9631ef`, `a8fa1874b`, `702d89b7a`, `9abb86bdc`, `cda7c0957`, `e79cad00a`, `40dcd9ac1`, `0bdf1d6a4`)
...
- [x] 35-20-PLAN.md — criteria 6/10/14 root causes: `steam` in the `RUNNERS` enum, one shared Steam launch dispatch that records a recent game, `installed.json` watcher notifies the renderer (wave 1) — complete 2026-08-30, 3 task commits (`b0b311321`, `bbed5f3e7`, `b6507de63`); does not itself close REQ-35-20 (needs the full live-gate re-run)
- [x] 35-21-PLAN.md — CR-01 `open_external` scheme allow-list (Rust) + CR-02 `frontendReady` once-semantics (wave 1) — complete 2026-08-30, 2 task commits (`94e5f88ac`, `fb12d4261`); both RED-proofs recorded verbatim in `35-21-SUMMARY.md`
- [x] 35-22-PLAN.md — CR-03 `window.platform` gains a `win32` arm + CR-04 renderer half: a failed logout reaches `gamelib.log` and the user (wave 1) — complete 2026-08-30, 3 task commits (`f20b90af6`, `bbbdb92fd`, `635151971`); both RED-proofs recorded verbatim in `35-22-SUMMARY.md`; does not close CR-04's backend half (plan `35-23`)
- [x] 35-23-PLAN.md — REQ-35-07: Epic per-domain cookie census, and the fatal-logout predicate distinguishes an empty jar from a broken clear (CR-04 backend half) (wave 1) — complete 2026-08-30, 2 task commits (`acf854233`, `b5ed7df03`); RED-proof recorded verbatim in `35-23-SUMMARY.md`; D-35-19-15 live proof still deferred to plan `35-29`
- [x] 35-24-PLAN.md — A-17 anti-rot: regenerate `i18nForkTouchedFiles.json` AND re-baseline the `--rewrite-scope` fixture counts AND the A-03 debt set, in ONE commit (wave 1) — complete 2026-08-30, 1 task commit (`ee86b3442`); re-measured from scratch rather than trusting the plan's own (now-stale) 199/163 baseline — real drift was 199 -> 205 fork-touched files (six Phase 35 files: `PathSelectionBox/index.tsx`, `WebviewControls/index.tsx`, `DownloadManager/index.tsx`, `CategoriesManager/index.tsx`, `InstallModal/defaultPlatform.ts`, `UseDarkTrayIcon.tsx`), all recorded as `DECLARED_UNSCANNED_DEBT` with named provenance, not folded into the hand-curated `meta/i18nGateScope.json` (verified byte-identical, still 163 files). Full `Meta` jest project 634/635 passing (1 pre-existing skip) and `pnpm codecheck` clean; three non-vacuity mutation controls proven and restored byte-identically. Does not itself close REQ-35-20 (test-infrastructure gap only, not the live-gate re-run).
- [x] 35-25-PLAN.md — REQ-35-16 winetricks: measure the renderer unmount, fix it, amend the unsatisfiable attribution clause (wave 1, **not autonomous**) — complete 2026-08-30, 2 task commits (`366e719bb`, `766ad63b5`); Task 1 measured a `WinetricksSearchBar` remount racing `mousedown`→`mouseup`, Task 2 fixed it by capturing Install on `mousedown`, Task 3's blocking human gate proved it live by mouse twice (`vcrun2005`, `vcrun2008`) with real `gamelib.log` installs; Task 4 amended `REQ-35-16`'s winetricks clause with superseded wording plus a dated correction scoped to the winetricks consumer only. Task 3's Library-consumer regression check DID NOT WORK but was dispositioned not a regression from this plan (`366e719bb` touches `SearchBar/index.tsx` with a comment only) — operator directed "proceed and file separately"; filed as its own unowned todo (commit `6d9584f75`) before Task 4 ran, explicitly excluded from this phase's gap-closure scope fence.
- [x] 35-26-PLAN.md — REQ-35-17 EOS: move the destructive confirmation renderer-side, fail-closed on anything but a literal `true` (wave 1, **not autonomous**) — complete 2026-08-30, 3 task commits (`81794b7bd`, `a5333be60`, `ad07e8ff6`); Task 1 moved `remove()`/`enable()`'s native confirmations off `dialog.showMessageBox` onto an explicit backend `confirmed: boolean` gated `=== true` (five separately-named refusal cases, RED-proven), Task 2 wired the renderer confirmations via `showDialogModal` on the house pattern and repaired `EosDeclineCallSiteGuard.test.ts`. **Task 3's live gate FAILED on attempt 1**: Settings -> Advanced's Install and Update buttons were wired bare with zero confirmation, a defect the existing guard structurally could not see (it censuses whether an already-wrapped call stays wrapped, not whether a button is confirmed at all) — 5/5 green tests, real data-loss-adjacent bug. Fixed by remediation commit `ad07e8ff6` (new `EosActionConfirmationGuard.test.ts`, RED-proven against pre-fix HEAD); **attempt 2 PASSED all steps**, independently corroborated against `gamelib.log`. Caveats recorded, not overstated: no screenshots captured (theme verdict is the operator's verbal confirmation, judged by eye); does not certify the `Dialog` primitive's general styling, only that EOS renders app-drawn not native. `D-35-11-01` RESOLVED. REQ-35-17 now COMPLETE (path-rejection + SEAM items already closed by `35-11`); ~13 native dialog sites outside EOS remain open, not claimed closed. See `35-26-SUMMARY.md`.
- [x] 35-27-PLAN.md — criterion 16 / `D-35-08-02`: split "is an operation pending?" from "which sleep kind should it block?" (wave 2, **not autonomous**) — complete 2026-08-31, 1 task commit (`0f5dfb352`; Task 1 was a pure decision checkpoint, no commit). Root cause: `allowedPendingOps` conflated "is an operation pending?" with "which sleep kind should it block?", so a still-running game kept `pendingOps > 0` and `window.api.unlock()` (gated on `pendingOps === 0`) never fired once its download ended — the download's `prevent-app-suspension` system assertion survived ~108s past the download's own end. Fixed entirely in `GlobalState.tsx` via option-a: new `classifySleepAssertionKind(status: Status)` (exhaustive, throws on an unrecognised `Status` rather than silently defaulting — mirrors REQ-35-06's Rust-side rule) splits `'launching'`/`'playing'` (display-kind only) from the nine download-shaped statuses (system-kind only), and `reconcileSleepAssertionCalls` applies an unlock-everything-then-relock-the-surviving-kind pattern against the existing, UNCHANGED `lock(playing: boolean)`/`unlock(): void` wire contract — `appShellFlowRegistration.ts` needed zero production changes since its per-id `powerId`/`displaySleepId` tracking already supports this call pattern. Both exposing faces (spurious acquire on a solo `'launching'` game, and delayed release on the packaged 12:06:45/12:08:04/12:09:33 timeline) independently RED-proven (both outputs recorded verbatim in `35-27-SUMMARY.md`); the fail-loud throw path for an unrecognised status also proven. New DOM-free `GlobalStateSleepAssertionClassification.test.ts` gets genuinely behavioural coverage of both pure functions via extract-transpile-eval (reads the real `GlobalState.tsx` source text, extracts each function's exact body by balanced-brace scan, transpiles with `typescript`, evaluates via `new Function`) rather than importing the module, which cannot load under this project's DOM-less frontend jest project. `src/backend/launcher.ts` untouched (`git diff` empty, per plan constraint). Does NOT itself close REQ-35-20 — the live packaged re-measure of criterion 16 is plan `35-29`'s, not this plan's. See `35-27-SUMMARY.md`.
- [x] 35-28-PLAN.md — records hygiene: correct the Phase 35 status column against evidence; nothing marked complete that cannot be cited (wave 2) — complete 2026-08-31, 1 task commit (`8def8e81b`). Re-derived all 21 REQ-35-01..21 status-table cells from disk rather than trusting this plan's own stale (pre-35-20/25/26) `<interfaces>` premise: 12 rows corrected to Complete (REQ-35-01/03/06/08/09/10/11/12/13/14/15/21, all `35-VERIFICATION.md` `✓ SATISFIED`), 2 to Partial (REQ-35-04/05 — code-level fixes landed in `35-20` but their live-gate criteria 6/10 are not yet re-measured, that re-run is `35-29`'s), and REQ-35-16 to a three-clause Partial reconciliation (openDialog Complete/live-proven; winetricksInstall Complete but SCOPED to the Winetricks consumer only per `35-25`, the shared `SearchBar` primitive is NOT certified and the Library consumer is separately broken/unowned; installed.json watcher's UI half is code-fixed by `35-20` but its live-gate criterion 14 still reads FAIL, re-run owed to `35-29`) — this corrects a disagreement between the plan's own stale premise (which assumed `35-25` had already discharged this clause) and `35-25-SUMMARY.md`'s own Task 4 text, which explicitly deferred the reconciliation here; disk won. REQ-35-20 untouched, stays Partial. Also reconciled `STATE.md`'s `completed_plans` counter (396 -> 399, +3 for the undocumented `35-21`/`35-22`/`35-23` bump gap routed here by `35-24` and `35-27`), with arithmetic shown inline in `STATE.md`'s progress block. Logged one out-of-scope, pre-existing `pnpm test --selectProjects Meta` failure (`genI18nGateScope` A-17, stale `i18nForkTouchedFiles.json` pin missing `WinetricksSearch/index.tsx` after `35-25` touched it) as deferred-items.md Item 6, not fixed.
- [ ] 35-29-PLAN.md — targeted live-gate re-run of criteria 6, 10, 14, 16 + criterion 21 with a seeded non-primary Epic domain, against a packaged macOS arm64 `.app` (wave 3, **not autonomous**)
...
⚠ **PREMISE CORRECTED 2026-09-01 (quick `260901-vm1`) — THE HARDWARE IS OWNED.** This goal line previously read "because it needs hardware or an OS this project does not have", and all 29 `blocked_by` values in `38-VERIFICATION.md` said the same. **That was false.** A Windows machine, a Linux machine and an Xbox controller (plus a second controller) are all on hand. **Not one item is blocked on an acquisition.** The cost is the *switch*: booting another machine, pairing a controller, and on four items registering a second Steam library. **This phase is schedulable at will — do not read it as waiting on the world.** The correction is not cosmetic: "needs an OS this project does not have" is unfalsifiable and so never gets scheduled or re-examined, whereas a switching cost is a price that competes for time like any other task. Every `platform_gate` expression is UNCHANGED and remains correct — those describe what the macOS dev machine can and cannot render, which is true regardless of what hardware sits in the room.
...
**Items: 23 OPEN as of 2026-09-23, plus 2 DISCHARGED and 9 RETIRED.** **SITTING 1 WAS HELD 2026-09-23 on the operator's Windows 11 machine** — the first live sitting this phase has ever had, and it discharged 2 items while turning up **4 NEW defects, none previously on record**: `checkNintendo` trusting a gamepad mapping it never checks, the Steam install-options caret opening ~1 click in 10, library cards rendering blank art permanently, and `isWritable_windows` returning FALSE for every path outside the user's own profile. `38-S06` PASS; `38-S08` FAIL on row 4 only (the section-gating matrix is CORRECT — the fault is `validPath` in the render condition, and row 4 must be re-scored after the `isWritable` todo lands, WITHOUT any change to `steamSectionGating.ts`). The controller leg was attempted and could NOT be scored: the operator's pad is a PowerA Advantage for Nintendo Switch 2 reporting `mapping: ""`, so `38-C01` needs SPLITTING at its d-pad/stick boundary and `38-C03`/`38-C04`/`38-C08` are unscoreable on shifted button indices. Full record in `38-HUMAN-UAT.md`. **The premise that this phase was waiting on hardware cost it four defects' worth of delay** — two of the four are not Windows-specific and were findable on the Mac all along. Was 25 earlier the same day: quick `260923-89z` RETIRED the nine electron-runtime items —
...
(plan `40-11`) verifies (`38-E03`); and embed drag-resize latency on hardware/backends other than
that same macOS host (`38-E04`). Honesty note distinct from every prior item here: `38-E01`/`38-E02`
are not "existing code awaiting a machine to render on" — `src-tauri/Cargo.toml` target-gates the
`unstable` `add_child` feature to `cfg(target_os = "macos")`, so no Windows or Linux code path
exists yet to observe; these are named unknowns (D-04), and resolving them is an implementation task
before it is a verification task. **`38-E03` and `38-E04` do NOT close on plan `40-11` passing** —
`40-11`'s live hardware gate verifies retina and drag-resize on ONE macOS test host only; its
...
- [x] 39-04-PLAN.md — collapse both `disconnect()` wipe-step sites (Humble + Epic) and disposition `seamBranchParity.test.ts` (REQ-39-03)
...
✅ **COMPLETE 2026-09-25 — 10/10 plans, 24/24 requirements, 0 gaps.** `43-VERIFICATION.md`
`status: passed`. The phase's one gap — REQ-43-24's `gog_keyless` "Claim on Humble" button, a
`major` live failure the operator observed (`43-UAT.md` item 8) — was root-caused, fixed
(`c99fdee43`: the row stopped opening the embed itself and the claim now routes through the Phase
40 host route) and **live-confirmed the same day**: Humble's keys page paints inside the app on
the Keys & Entitlements tab, logged in, no browser launched, zero errors. Its todo is closed.

This entry read `🚧 … ONE OPEN GAP` for part of 2026-09-25; that marker is superseded, not
deleted, because the gap it named was real. Limits of the live confirmation are recorded in
`43-VERIFICATION.md` rather than here: debug build not packaged, the button's own `onClick` not
clicked (needs an unlinked GOG account; pinned by tests instead), bounds-sync under resize
untested. Full detail in the superseding note at the end of this entry.
...
**Plans:** 7/7 plans executed — **PHASE 46 COMPLETE, closed 2026-09-25.** Live re-gate `46-LIVE-GATE-RERUN.md` PASS (first gate `46-LIVE-GATE.md` FAIL at Check 3, fixed by 46-06); `46-VERIFICATION.md` passed 12/12; `46-REVIEW.md` 0 critical / 2 warning / 2 info (advisory).

Plans:
**Wave 1**

- [x] 46-01-PLAN.md — Wave 0 `cargo test` compile fix on Windows + TDD the seven pure guard helpers (SID key, mutex/pipe names, SDDL, retry, owner match, payload) + file Windows-CI todo

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 46-02-PLAN.md — windows-sys dependency + `#[cfg(windows)]` mutex/pipe acquisition, authenticated secondary delivery, and main() wiring before `Builder::default()`

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 46-03-PLAN.md — Named-pipe accept loop in `.setup()` (re-validated through `protocol_url_arg`) + Phase 46 source gates incl. REQ-46-08 `on_open_url` ordering

**Wave 4** *(blocked on Wave 3 completion)*

- [x] 46-04-PLAN.md — ONE commit: delete the `tauri.windows.conf.json` schemes override + invert `windowsDeepLinkSuppression.test.ts` + REQ-46-06 pin + comment rewrites; then debug NSIS build and `installer.nsi` 6-line proof
...
- [x] 46-07-PLAN.md — Wave 2 (gap), NON-AUTONOMOUS: live re-gate `46-LIVE-GATE-RERUN.md` (P0, Checks 1, 2, 3a terminal, 3b Start menu, 4, 5; S1 tray non-gating) + verdict-gated closure of the todo, U-34.5-18 and REQ-46-10
