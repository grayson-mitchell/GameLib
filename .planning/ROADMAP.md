...
### v0.8 — Rust/Tauri Rearchitecture (exploratory)

- [ ] **Phase 27: Tauri Shell Walking Skeleton** - The thinnest end-to-end slice of the Rust/Tauri target architecture running on macOS: a native Tauri shell hosts the unmodified React renderer, the existing Node backend runs as a supervised sidecar, `window.api` is served by re-pointing the 3 preload factories, and two real flows work (Steam library renders from the sidecar; a steam:// launch fires via opener parity). Additive/reversible — Electron build stays intact. Sourced from spikes 009–012. (registered 2026-07-20)
...
**Goal:** Stand up the thinnest possible **end-to-end** slice of GameLib's Rust/Tauri target architecture and prove it runs on real macOS. A native **Tauri v2** shell (Rust) hosts the **existing, unmodified** Vite React renderer; the **existing Node backend** runs as a **supervised Tauri sidecar** (bundled Node, not rewritten); `window.api` is served by **re-pointing the 3 preload factory functions** onto a Tauri↔sidecar transport (379 call-sites untouched); and **two real flows** work end-to-end — the Steam library list renders from the real `steam-user`/store-manager path in the sidecar, and a `steam://` launch fires through `tauri-plugin-opener` parity. The Electron build stays intact — this phase is **additive and reversible**.

**Status:** Registered 2026-07-20 from spike findings 009–012 (Idea C). Scope = walking skeleton (user decision); context sourced from spikes, no discuss-phase. This IS the recommended 5th integration spike, productionized.

**Depends on:** Phase 1/2 (Steam auth + library — the `steam-user` read path reused in the sidecar) and the Idea-C spike line (009 backend-headless-sidecar, 010 steam-user-vs-sidecar, 011 electron-api-parity, 012 react-frontend-under-tauri).

**Blueprint (LOCKED, from spikes):**

  - **Backend as sidecar (009):** 80% of backend files are Electron-free and port as-is; but the entrypoint faults at *import time* — `app.getPath()` runs at module scope and `electron-store` throws at construction. The sidecar entry must shim path resolution + a minimal store replacement before importing backend modules.
  - **Steam stays JS (010):** keep `steam-user` + depot pipeline in the Node sidecar (0 electron imports, runs headless). Do not rewrite in Rust.
  - **Bridge = 3 factories (012):** the whole `window.api` is generated by `makeHandlerInvoker`/`makeListenerCaller`/`frontendListenerSlot`. Re-point only these onto Tauri `invoke`/`emit`/`listen`. 5 renderer files import electron directly — route them through `window.api`.
  - **Platform parity proven (011):** `shell.openExternal(steam://)` → `tauri-plugin-opener`; `safeStorage` → `keyring` (macOS Keychain). Both proven live in compiled Rust.

**Success criteria:**

  1. A dev command launches a native Tauri window showing the real GameLib UI.
  2. The Steam library list is populated by the Node sidecar (real data path, not mocked).
  3. A Steam launch triggers a real `steam://` handoff via the opener parity path.
  4. Zero changes to the 379 `window.api.*` call-sites; only the 3 factories + a new transport module change on the renderer side.
  5. The existing Electron build still works (port is additive this phase).

**Out of scope (later phases):** the full 220-endpoint IPC re-plumb; project-wide electron-store replacement + user-data migration; the 44-file lifecycle/dialog/tray/updater cluster; Windows/Linux packaging, auto-update, signing/notarization; Electron removal; `session`/`powerSaveBlocker` shims.
