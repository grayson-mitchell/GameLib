...
### RETEST RUN 3 (2026-07-21 ~19:08, .app rebuilt 19:02 with gap 24-17) — Gate 3 (Avernum 6) PASS; Gate 4 (Hoard) reveals shim interface-coverage limit

24-17 (bridge-authoritative install-state) CONFIRMED working: Avernum 6 + Hoard both showed **Install** (no longer shadowed by native/Phase-17-bottle), bridge-installed cleanly into `GameLibSteamBridge` (all three ACFs + game dirs present), shim overwrite verified (Hoard `win32/steam_api.dll` = 805888 B, `127.0.0.1`=1).

**✅ GATE 3 (R6, Avernum 6) — PASS.** Avernum 6 reached **playable single-player through the bridge** on real Apple-Silicon hardware (user-confirmed "works"). First real acceptance game fully playable via the out-of-process bridge. Combined with Gate 1 (vtable identity round-trip) this validates the bridge MECHANISM end-to-end: install → shim overwrite → CrossOver launch → `SteamAPI_Init` through the helper → game runs. Avernum 6 has a minimal footprint (Init/Shutdown/RunCallbacks) so it does not exercise the interface vtables heavily — that's expected.

**⚠ GATE 4 (R6, Hoard) — BLOCKED on shim INTERFACE COVERAGE (not a bridge defect). New finding D-UAT-24-09.** Hoard launched through the bridge (shim loaded, game ran) then crashed: `Unhandled exception: unimplemented function steam_api.dll.SteamUtils called in wow64 32-bit code`. `objdump` on `win32/Reuben.exe` shows Hoard imports **8 bare old-style interface accessors**: `SteamUser`, `SteamUtils`, `SteamFriends`, `SteamApps`, `SteamMatchmaking`, `SteamNetworking`, `SteamRemoteStorage`, `SteamUserStats` (plus the lifecycle SteamAPI_Init/RunCallbacks/RegisterCallback/etc. which the shim DOES export). The generated shim (`steam_api.def`) exports only the lifecycle funcs + the MODERN versioned `SteamAPI_SteamUser_v023` + `SteamAPI_SteamFriends_v018`; it exports NONE of the bare accessors, and the helper marshals only **ISteamUser + ISteamFriends** (2 of Hoard's 8 interfaces). So Hoard hits the first unimplemented accessor (SteamUtils) and wine aborts. **This is NOT the bridge failing — it's the interface surface being deliberately scoped to 2 interfaces (the phase-24 spike set).** Fully supporting Hoard requires: bare-accessor exports for all 8 interfaces + building 6 NEW interface proxies (ISteamUtils/ISteamApps/ISteamUserStats/ISteamRemoteStorage/ISteamMatchmaking/ISteamNetworking) with vtable marshaling in both the generated shim (24-01 generator) and the helper (24-02) — a substantial extension, realistically a follow-on milestone/phase, NOT a gap-cycle tweak. Hoard was flagged "PARTIAL" in the allowlist note; this quantifies it.

**Disposition:** Bridge MECHANISM is PROVEN (Gate 1 vtable identity + Gate 3 Avernum 6 playable). R6's literal "Avernum + Hoard both playable" is met for Avernum (6) but Hoard needs broad interface coverage that is out of Phase-24 scope. Recommend: accept Avernum 6 as R6's bridge proof; scope multi-interface coverage (incl. Hoard) as explicit future work; handle the allowlist so Hoard is not offered as a working bridge title until covered (mark partial / gate behind a coverage check, so users aren't handed a game that installs then crashes).
