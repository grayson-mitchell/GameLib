...
- truth: "When a Steam game needs an update before launch, GameLib should give the user feedback (rather than showing 'Launched in Steam' and dismissing while Steam silently updates)."
  status: deferred
  reason: "User observation (test 5): no update feedback in GameLib; user must open Steam to discover the game is updating. DEFERRED to backlog (post-v0.2) — new capability, and Steam does not report update state back to GameLib, so this needs its own design. NOT part of Phase 8 gap closure."
  severity: minor
  test: 5
  artifacts: []
  missing: []
