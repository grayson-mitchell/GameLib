...
human_verification:
  ...
  - id: "38-C01"
    sitting_1_2026_09_23: "ATTEMPTED, NOT SCOREABLE AS WRITTEN. Pad: PowerA Advantage Wired Controller for Nintendo Switch 2. The LEFT STICK half works (focus moves in all four directions); the D-PAD half FAILS -- the d-pad produces no response at all because this pad reports it as a hat axis (`axes[9]`) while `checkNintendo` reads `buttons[12-15]`. This item names BOTH inputs, so per relocation_rules (4) it must be SPLIT before either half is recorded -- a single pass/fail here would make the un-run half disappear. See todo `2026-09-23-checknintendo-trusts-standard-mapping-on-non-standard-pads.md`."
  ...
  - id: "38-C04"
    sitting_1_2026_09_23: "ATTEMPTED, UNSCOREABLE. Depends on `back` plus the STICK CLICKS (`buttons[10]`/`buttons[11]`) on a pad whose button indices are known shifted. Observed live: physical A fired `altAction` (launching the game) while the hint bar read 'A: Game details', and physical B fired `mainAction`. Recording a pass or fail against that would bake the pad defect into this item. See todo `2026-09-23-checknintendo-trusts-standard-mapping-on-non-standard-pads.md`."
  ...
  - id: "38-C05"
    ...
    scope_note: '34.1''s item 7 note already identified this as THE SAME unmeasured surface as 38-C01..C04, not a coincidence — no phase since has had a controller available. Run C05 in the same sitting as the other four.'
  ...
  - id: "38-C06"
    ...
    why_human: "Requires a physical controller."
    blocked_by: "controller pairing -- pair the Xbox controller (OWNED and available; the cost is the pairing, not the hardware)"
    platform_gate: '`.NavShell__tier2Portal` (src/frontend/components/UI/NavShell/index.scss:499-505) is the panel''s OWN scroll container -- `overflow-y: auto`, nested inside `.NavShell__tier2`''s `overflow: hidden` (:401), and outside `main.content` entirely. No focus-scroll handler is bound to it: `grep -rn "scrollIntoView|addEventListener(''focus''|onFocus"` over `NavShell/` and `Header/` returns ZERO hits. Focus dispatch itself is the same gate as 38-C01..C05 -- `gamepadAction` is dispatched only from the `navigator.getGamepads()` polling loop (src/frontend/helpers/gamepad.ts:559,678), so nothing here is reachable from a keyboard.'
    origin_phase: "34.11"
...
human_verification_discharged:
  - id: "38-S06"
    ...
    prior_state: 'Pending in 34.13''s ledger from 2026-08-15 to 2026-08-28. Never attempted: this project''s only host is an Apple Silicon Mac, so the branch never rendered. Relocated at gate close-out once every macOS-runnable row was resolved (44 PASS / 0 FAIL).'
  - id: "38-S08"
    result: "FAIL on row 4 only -- sitting 1, 2026-09-23, Windows 11, tauri dev build at `6ad1d7cd9`, native installs ON with the operator's two real Steam libraries. Platform row PRESENT (pass), library dropdown PRESENT (pass), wine section ABSENT (pass), FREE-SPACE LINE ABSENT where PRESENT is required (FAIL). THE MATRIX IS NOT AT FAULT: `steamSectionGating.ts:284` sets `freeSpaceLine = libraryDropdown`, so the verdict is correctly true; the render adds three conditions this item never mentions (`gating.freeSpaceLine && diskSpace && diskSpace.validPath && diskSpace.validFlatpakPath`, SteamDialog/index.tsx:493-497) and `validPath` is false. Root cause PROVEN by running the same `Get-Acl` the backend runs: `isWritable_windows` matches ACLs by INDIVIDUAL username and neither Steam library carries a per-user ACE."
    requires_rescore_after: "todo `2026-09-23-iswritable-windows-only-true-inside-the-user-profile.md`. When that lands, row 4 must be re-scored on a Windows host. It should pass WITHOUT any change to `steamSectionGating.ts` -- if a fix there is proposed instead, the diagnosis was wrong."
