...
- **D-04:** **GameLib owns resume/interrupted-download recovery**, not just the happy-path first install. A resumed download re-verifies every chunk (sha1) and re-applies file modes, and if it can prove the install is complete it writes a trustworthy `StateFlags=4` — the same guarantee as a fresh install. This is a genuine scope expansion beyond spike-003's minimum (partial-state tracking + re-selection/reconciliation logic) — flag for research/planning as the largest new lift.
  - **SOFTENED 2026-07-18 (debug: `steam-startup-resume-crash`):** the *trigger* for resume recovery changed from unattended-on-launch to user-initiated. `SteamLibraryManager.init()` no longer auto-drives `locate -> rebuild-plan -> reconcile -> finalize -> watch` for a leftover `StateFlags=1026` install found on startup — a native fatal in that machinery running immediately after launch (PICS/manifest fetch + depot download/decompress) was crashing the whole app on every boot when a stale interrupted install existed. init() now only DETECTS the interrupted install and flags the library entry `steamResumePending: true` (surfaced to the user, no heavy work runs on boot). The exact same recovery logic (still GameLib-owned, still re-verifies every chunk, still re-applies file modes, still can earn a trustworthy `4`) now lives in `resumeInterruptedSteamInstall()` (`src/backend/storeManagers/steam/library.ts`) and only runs when the user's own Install click triggers it (`SteamGame.install()` in `games.ts`). Ownership of the recovery logic is unchanged — only the automatic-vs-consent-gated trigger is softened. See `.planning/debug/resolved/steam-startup-resume-crash.md`.
