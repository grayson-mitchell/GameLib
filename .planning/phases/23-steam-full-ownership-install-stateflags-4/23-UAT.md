...
- id: G-23-01
  ...
  status: resolved  # 2026-08-19 -- fix shipped in Phase 23.2 AND now proven on real hardware; see proven_by below
  proven_by: "REAL macOS HARDWARE, 2026-08-19 17:16:39-17:17:54 NZST, GameLibSteam CrossOver bottle, live Steam CM/CDN -- recorded in .planning/phases/23.2-.../23.2-HUMAN-UAT.md items 1 and 2 (both `pass`). A GameLib install of appId 1771300 was triggered with Steam refusing depot 1771304's key exactly as in the 2026-07-21 abort, and THE INSTALL COMPLETED. Log evidence: 17:16:44 `fetchDepotPlanEntry: couldn't get decryption key for depot 1771304 (app 1771300): eresult=40` followed immediately by `buildDepotPlan: skipping depot 1771304 ... the install is continuing without it`; 17:16:45 `buildDepotPlan: total 5953ms for appId 1771300 (3 depot(s))`; 17:17:51 `Writing StateFlags=4 full-ownership manifest for appId 1771300 (sizeOnDisk=96422090071, buildid=23914554)`; 17:17:54 `install polling complete -- badge flipped to installed`. The resulting bottle appmanifest_1771300.acf matches the official Windows client's on EVERY benchmark field: StateFlags 4, SizeOnDisk 96422090071, buildid 23914554, InstalledDepots block byte-identical (1771302/1771303/1771306, same manifest GIDs and sizes), 1771304 ABSENT. SizeOnDisk is MEASURED from disk (measureInstalledBytes, depot.ts:797-798), not summed from the plan, so the match is an independent check. The user was also shown the skip notice for real (item 2): THREE distinct OS notifications coexisted -- the depot-skip notice, `Restart Steam to finish installing`, and `Installation Finished` -- which additionally proves in production that the skip notice's `notifiedDepotSkipped` flag is genuinely separate from `notifiedWaiting` (the D-06/D-07 shared-flag trap), something jest could not establish."
  resolution_method_note: "The 90GB was NOT re-downloaded. The official client's content was already on disk, so its appmanifest was moved aside (three md5-verified copies kept) and GameLib re-installed over the existing files; reconcilePartialState (reconcile.ts:126-157) sha1-verified all 138 manifest entries and produced jobCount=0 / reconciledSkipped=138, completing in 71.5s. This is a RESUME, not a fresh download -- stated as an honesty limit on the UAT item. It fully exercises the defect this gap names, because the 1771304 key refusal happens in buildDepotPlan BEFORE any bytes transfer, and it also exercises the completeness gate and the manifest write. It does not re-prove the bulk download path, which 23.2-CONTEXT.md forbids re-running."
  why_was_still_open_DISCHARGED_2026_08_19: "[HISTORICAL -- the condition below was met on 2026-08-19; retained so the bar this gap was held to stays legible.]"
  fix_shipped_23_2: "Phase 23.2 EXECUTED 2026-08-19 (4/4 plans, code review clean, full suite 295 suites / 6169 tests green). Both of this gap's `missing:` items are discharged: the policy was DECIDED (23.2-CONTEXT.md D-01..D-07) and IMPLEMENTED (buildDepotPlan now reduces the plan on an EResult-40 key/manifest refusal and continues; skippedDepots threaded to a completion notice under the new i18n key steam.download.notify.depotSkipped). Only EResult 40 skips -- {8,9,15,17,42,43} still abort by design, pinned by a parameterized negative test RED-proven by temporarily widening the comparison. select.ts, depotErrors.ts and canWriteFullOwnership are byte-identical to the planning commit. An all-skipped plan can no longer report success (REQ-23.2-05, depot.ts:811) -- a hole no prior artifact had named."
  why_still_open: "This gap's truth is a claim about a REAL INSTALL COMPLETING, and it has been proven only in jest. Nothing in Phase 23.2 touched hardware: no KCD2 install was run (the diagnostic was deliberately read from the official client's existing artifacts rather than re-downloading 90GB), so 23.2-CONTEXT.md's own acceptance benchmark -- that a GameLib install of 1771300 should now produce InstalledDepots {1771302, 1771303, 1771306} at SizeOnDisk 96422090071 -- is UNOBSERVED. The bar for `resolved` in this document is set by sibling gap G-23-02, which carries a `proven_by:` naming real macOS hardware; this gap has no such line and must not claim one. Closing it on a green suite would repeat this project's standing failure mode (green jest != gate satisfied). Resolve only after a live install of 1771300 is observed to complete with 1771304 skipped and the user actually shown the skip notice."
  honesty_limit_carried_forward: "Unchanged by 23.2: the evidence proves 1771304 is not NEEDED; it does NOT prove Steam would have granted its key -- the official client never asked. The shipped skip-and-warn copy is worded accordingly."
  ...
  missing: []  # both items below DISCHARGED 2026-08-19 -- retained for provenance
  missing_DISCHARGED:
    - "[DONE, Phase 23.2] Decide policy: should a Blocked key on a non-essential owned depot skip-and-warn (continue install) rather than abort? Requires distinguishing required vs optional/region-alternate depots at selection time. -- DECIDED in 23.2-CONTEXT.md D-01..D-07 and shipped; note the policy deliberately does NOT classify essentiality at selection time (REQ-23.2-02, select.ts unchanged); reduction happens downstream in buildDepotPlan's loop."
    - "[DONE, Phase 23.2] UNGATED 2026-08-19: the required-vs-optional depot selection-policy follow-up in deferred-items.md was released by the Task 3b verdict and ready to plan as its own gap cycle. -- Planned and executed as Phase 23.2 (4/4 plans), verified 8/9 at jest level, and the remaining 1/9 (the live acceptance benchmark) was observed on real hardware 2026-08-19; see proven_by above. The 1771306 half of the scope was already DISPROVEN as a divergence (see second_divergence_RESOLVED)."
