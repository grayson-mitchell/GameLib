...
- truth: "A Humble sync updates Keys waiting to final categorization without visible intermediate churn (all keys appearing then progressively removing/recategorizing)"
  ...
  severity: minor
  test: 8
  root_cause: "Hypothesis (code-evidenced, not yet confirmed): library.ts broadcasts humbleKeysUpdated after EVERY committed order (D-26 progressive fill - library.ts:798), and the classifier v4->v5 bump forced reclassifyAll over every cached order, so the renderer receives many intermediate snapshots where later orders are still unclassified/stale and ownership dedup has not yet recomputed - keys transit Keys waiting before settling. Related: .planning/debug/humble-keys-empty-list-flashing-sync.md (awaiting_human_verify) documents the same D-26 progressive-fill broadcast causing sync flicker (S2). RULED OUT one-time migration artifact: user re-ran a normal post-migration sync, same fill-then-empty churn. Candidate mechanisms needing debugger confirmation: (a) classifier version stamp not persisted after v5 pass, so reclassifyAll re-runs every sync; (b) WR-01 fix (patchCachedState allTerminal recompute / unfreeze) unfroze orders so every sync re-fetches them; (c) ownership dedup recompute runs AFTER order commits, so owned/spares keys transit Keys waiting on every sync before dedup moves them; (d) per-order humbleKeysUpdated progressive fill (D-26) amplifying whichever of the above."
  ...
  missing: ["Confirmed root-cause mechanism (debugger investigation), then fix: likely batch/defer humbleKeysUpdated until classification+dedup settle, and/or repair version-stamp/freeze persistence if reclassify re-runs every sync."]
