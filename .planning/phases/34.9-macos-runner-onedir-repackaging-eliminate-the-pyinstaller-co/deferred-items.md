...
**PASTE HALF — VERIFIED-ABSENT 2026-08-26:** the paste half of this item **did not reproduce** and is closed on evidence. Scope note, binding: this line disposes of the **PASTE HALF ONLY** and makes **no claim whatsoever about the Enter half**, which is separate work owned by `34.17-01` / `34.17-02`. Run under `34.17-03`'s pre-authored contract (`34.17-HALF-B-PASTE-GATE.md`, RUN RECORD fields authored empty before the run), on macOS 26.5.2 arm64 at HEAD `614a8c82e`, session dir `/tmp/gamelib-34.17-halfb-Z6sjZY`. A 30-byte ASCII probe was loaded and verified via `xxd` before the run. Cmd+V into `#setting-alt-legendary` delivered `/tmp/gamelib-paste-probe-12345` **character-exact**; the uncontrolled SearchBar (item 3) and the menu-bar Edit ▸ Paste route (item 5) were likewise character-exact. Verdict-table row 1 (item 1 clean) ⇒ `VERIFIED-ABSENT`. **Both named candidates are excluded by evidence, not assumption:** Candidate A (`navigator.clipboard` no-op) has no mechanism reaching a native paste into a DOM `<input>`, and Candidate B (tao consuming the Cmd+V key-equivalent) predicts silence whereas all 30 bytes arrived. Evidence: `.planning/phases/34.17-.../evidence/34.17-halfb-target-cmdv.png`. **The 2026-08-11 report is not contradicted as having been made in good faith** — it was never independently re-confirmed, per this item's own caveat, and this run establishes only that it does not reproduce on this host at this commit.

**ENTER HALF — FIXED 2026-08-26:** the Enter half of this item is closed by code. `PathSelectionBox` now commits the typed value on Enter through `TextInputWithIconField`'s already-declared `onKeyDown` prop (no interface widening) — the prop reaches the real DOM `<input>` by double spread: `TextInputWithIconField/index.tsx:24,28` spreads `...props` into `TextInputField`, which collects `...inputProps` at `:30` and spreads them onto the `<input>` at `:65`. Landed in `34.17-01` with an 8-case no-DOM jest suite; a double-commit guard (path-prop comparison + one-shot value-scoped Enter-then-blur suppression) stops `EgsSettings.tsx` spawning `legendary egl-sync` twice for one edit. `34.17-02` added the missing affordance the original item called for — a reserved hint row reading "Not saved yet — press Enter" while the buffer diverges, and a ~2s "Saved" confirmation after a commit fires. **This disposition line was added during PHASE VERIFICATION, which found it missing:** neither `34.17-02` nor `34.17-03` required it — `34.17-03`'s contract scoped its ledger obligation to the PASTE HALF only, so the fixed half had no owner for its own closure line. The gap sat between two correct plans, not inside either.
...
**CLOSED 2026-08-27 (Phase 34.18, plan 34.18-07):** `34.18-LIVE-GATE.md` returned verdict **PASS, 21/21 criteria**, following the developer's dispatch of `build-runners-onedir-macos.yml` run `33047720816` (`conclusion: success`) and a pin against its published digests. This item's own mechanism — `install-deps`'s `pnpm download-helper-binaries` step throwing on the `PENDING-CI-PUBLISH` sentinels, so the macOS CI leg never reached the `verify:runner-bundle` guard — is discharged: the three arm64 sentinels are gone, replaced by real digests pinned from run `33047720816` (§2), and `pnpm download-helper-binaries` exits 0 on darwin with no sentinel throw (§3 criterion 1). The fail-closed path was separately proven by the corruption test: a one-character digest flip produced a non-zero exit naming both the corrupted archive and run `33047720816` (§3 criterion 5), and the digest-mismatched archive was not unpacked under `public/bin/arm64/darwin` (§3 criterion 6 — recorded with its literal over-scope against `public/bin` noted, not silently re-scoped: the unrelated linux/win32 runner trees the literal command also matched are a separate, non-onedir download path and out of this item's scope).

**What was NOT observed, stated so it is not overstated: the guard has still NOT been exercised in a CI job.** Run `33047720816` executed only `build-runners-onedir-macos.yml`'s two jobs (`prepare-release`, `build (macos-14, arm64)`) — it built and published the runner archives; it did not invoke `pnpm dist:mac`, `pnpm release:mac` or `pnpm verify:runner-bundle`, and no `build-base.yml` macOS leg ran. This item closes on **the sentinel blockage being removed** — the specific mechanism this item names as CI's actual blocker — not on the guard having run in CI. That remains REQ-34.16-02's open half, updated separately in `.planning/REQUIREMENTS.md` and NOT marked complete by this closure.
