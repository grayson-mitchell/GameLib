...
- [ ] **REQ-30-08**: The minimum read-only channels the **Steam depot path's** `DownloadDialog` needs to render and submit are ported, and the phase ships an **enumerated list** (not prose) naming every channel it ports — QR login, install/uninstall/updateGame/checkGameUpdates, and the modal read subset — with those entries moved out of `SEAM.md` §3's deferred table into §1. Channels outside the declared set keep rejecting non-fatally. *(D-10, D-11 + checklist step 5)*
