...
- [ ] **REQ-30-07**: `dialog`'s open-directory path gets **real behavior** in `electronStub.ts`, bound to a Tauri command through the existing generic `requestRustInvoke()` / `dispatch_rust_channel()` channel — no bespoke correlated-request mechanism is invented. `notify()` remains a no-op that **logs**, not a silent one. *(D-09 + checklist steps 3 and 6)*
