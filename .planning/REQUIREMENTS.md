...
- [ ] **REQ-24-07**: Clean fallback + coexistence with Phase 22 — the bottled-Steam path remains fully functional for non-allowlisted titles; a bridge failure for an allowlisted title surfaces a clear, non-silent error/fallback (D-05/D-11). — Partial (2026-07-20, Plan 24-08): `games.ts` routing/fallback-trigger wiring now built on top of Plan 24-06's `ensureBridgeHelperReady()` readiness precondition — every terminal bridge-failure branch (provisioning, depot download, exe resolution, shim placement, launch) calls `markBridgeFailedThisSession(appId)` and fires `steamBridgeSetupRequired`; `isBridgeEligible()` consults that session set so a fallback re-invocation of `install()`/`launch()` routes to the existing bottled path instead of looping back into the bridge (finding #3, regression-tested). A non-allowlisted title's install/launch/uninstall is confirmed byte-for-byte unchanged from Phase 17. The D-05 fallback DIALOG UI itself (and D-11's on-demand Phase-17-bottle provisioning) remain deferred to Plan 24-09.
...
| REQ-24-07 | Phase 24 | Partial (2026-07-20, Plan 24-08 — games.ts fallback-bypass routing (markBridgeFailedThisSession + finding #3) built on Plan 24-06's ensureBridgeHelperReady() readiness seam; D-05 fallback dialog UI + D-11 on-demand Phase-17-bottle provisioning deferred to Plan 24-09) |
