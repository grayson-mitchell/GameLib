...
- [ ] **REQ-34-09**: Live phase-close gate (Manual-Only). A real `v*` test-tag push proves all three matrix legs complete, a Draft+prerelease Release appears with the 3 platform artifacts + `latest.json`, it is NOT marked GitHub "Latest" and is invisible to the updater until manual publish (Phase 19 lesson), signing is gracefully skipped (unsigned artifacts + warning, job green), and the compiled sidecar runs standalone with no system Node on PATH. Jest cannot exercise a GitHub Actions run or a Node-free container — this is a `checkpoint:human-verify` gate. *(D-04, D-09)*
