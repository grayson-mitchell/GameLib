...
- [ ] **REQ-34-07**: keyring crate cross-platform features. `src-tauri/Cargo.toml`'s `keyring` dependency adds `windows-native` + `sync-secret-service` alongside the existing `apple-native` (no major-version bump) so OS-keychain Steam-token persistence works on Windows/Linux, not just macOS; target GNOME/KDE Secret Service on Linux (runtime "no keyring found" surfaces as the existing structured `keyring:unavailable` error, not a crash). *(implied by D-01; surfaced by 34-RESEARCH.md)*
