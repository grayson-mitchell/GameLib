...
---
*Last updated: 2026-07-08 — v1.2 Phase 14 (Guided Claim Flow) complete: HumbleClaimWizard with danger-gated single-key reveal (no auto/bulk reveal), C2 hard block routing owned games to Giftable Spares, write-ahead audit log before every reveal POST, clipboard + Steam registerkey deep-link + mark-redeemed/undo cycle, non-Steam link-out without one-click activation. Reveal contract live-validated with a disposable key (CSRF required; Electron net.request transport — axios blocked by Cloudflare). HCLAIM-01..05 validated; verification passed 5/5. Fast-follow debt: 14-REVIEW.md CR-01 (sync drops local-redeem mark, breaks Undo) + WR-01..06 need accept-or-remediate before ship. v1.1 kept open (Phase 9 + Phase 7 UAT outstanding) by explicit choice.*
