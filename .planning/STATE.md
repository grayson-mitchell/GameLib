...
Plan: 3 of 3
Status: Phase 25 shipped — pushed to gamelib, folded into PR #3
Last activity: 2026-07-19 - Completed quick task 260719-t8t: native Steam install window (GOG-styled)
...
| 260719-t8t | Native Steam install window (GOG-styled): every native Steam install now ALWAYS opens a single `SteamDownloadDialog` (art+title header, registered-library `<select>` per D-08, live free-space via `checkDiskSpace` recomputed on library change, best-effort download size → literal "Unknown", Install→`installSteamGame`), reversing the D-09 zero-friction 0/1-library immediate-install path and retiring the bare `SteamInstallLocationPicker`. Does NOT call `getInstallInfo` (avoids Steam "Getting download size…" infinite-loop). Frontend-only; `tsc --noEmit` 0, frontend jest 158/158 (2 new suites: 6+11). On its own branch `feat/steam-native-install-window` (rebuilt onto base `bee590f1`: task-1 + task-2-with-picker-deletion + docs only). NOTE(resolved): a concurrent bottle-debug session's commit had tangled the D-UAT-10 bottle fix + the staged picker `git rm` onto this branch; branches were sorted — the bottle code fix lives on `fix/steam-native-install-stability` (`4b87d58d`, already present), the picker deletion stays folded into this feature's task-2 commit, and the unrelated game-art fit-to-size experiment moved to its own branch `feat/fallback-art-fit-to-size`. | 2026-07-19 | [260719-t8t-steam-native-install-window](.planning/quick/260719-t8t-steam-native-install-window/) |
