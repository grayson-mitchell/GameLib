...
LIVE APPEARANCE NOT VERIFIED -- 5 human-gate items OWED, the silent-regression one being "no gap
at the top when unfiltered" (reasoned from the `activeFilterCount === 0` self-suppression, not
observed). Prior: quick task 260815-opt: two filter-visibility fixes for the case where
