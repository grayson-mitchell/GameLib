...
last_activity: "2026-08-21 (NZST) -- Quick task 260821-o34 switched the GOG user API off embed.gog.com/userData.json, which fails outright (errors rather than truncating) for accounts with large wishlists because it embeds the whole wishlist/friends/checksum payload; the symptom was a user who had authenticated successfully still appearing logged out, since getUserDetails() returned undefined and configStore.userData was never written. GOGUser.getUserDetails() now fetches https://api.gog.com/users/${encodeURIComponent(userId)}, a small fixed-size document. A blast-radius sweep of all seven consumer sites proved only username/galaxyUserId/userId and the presence of the key are ever read, so UserData was NARROWED from 20 fields to those three rather than marking the dropped fields optional -- optional still compiles userData.friends?.length against data that can never arrive, narrowing makes tsc reject it; Currency and Friend went with it. The accessToken?: string param became credentials?: Pick<GOGCredentials, 'access_token' | 'user_id'> so login() reuses the user_id its own gogdl auth --code exchange already returned -- NO new gogdl spawn, preserving the measured ~5s post-login saving from debug/gog-spawn-reduction.md. galaxyUserId/userId come from the resolved credentials and never the response body (T-Q34-01), so a manipulated response can only change a displayed username, never redirect a gameplay.gog.com playtime session URL; the unprovable-offline assumption that gogdl user_id == the old galaxyUserId is handled by a drift WARNING naming both ids rather than an assertion. Both original token-reuse pins were retargeted (not deleted) to the full scripted-user_id URL so they now also prove the threading, plus 4 new regression cases. The orchestrator re-ran both gates itself rather than trusting the executor report: pnpm codecheck exit 0, user.test.ts 11/11. Commits b0776ab8d, 463426e9d, e54f04120; the originating todo was closed separately by a concurrent session in b5ef5d913."
progress:
