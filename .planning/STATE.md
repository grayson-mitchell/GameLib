...
| 260820-kq0 | **Steam sign-in now presents as an in-app modal window** — rendered inside the shared `frontend/components/UI/Dialog` primitive (centred, full-viewport backdrop, titled header, X/Escape/backdrop dismissal) instead of the bare `.steamLoginPanel` box pinned flush to the top-left of the content area. **The request as literally stated was not achievable:** Humble opens a REAL native Tauri child window (`humble_login_open`), but login child windows are deliberately given ZERO Tauri command access (`src-tauri/src/main.rs:967` — child labels "never join `capabilities/default.json`'s `windows: ["main"]` array"), while Steam authenticates in-process via `window.api.steamStartQR`/`steamStartCredentials`/`steamSubmitGuard` — so moving Steam into that window arm would have required widening the capability scope, a security regression. The operator was shown both readings (in-app modal vs. a native child window driven over the `LOGIN_WINDOW_EVENTS` seam) and **explicitly chose the in-app modal**; the native-window reading remains unbuilt and is the option to revisit if true OS-window parity is ever wanted. Second finding that shaped the plan: giving the Steam tile a `primaryLoginAction` to drive an overlay would have FALSIFIED `loginInFlightUiReachability.test.tsx`, the mutation-proven source gate for T-34.4.2-39/-41 whose documented mechanism is that clicking a login tile navigates away and unmounts all six tiles — so `Login/index.tsx`, `Runner/index.tsx` and `App.tsx` are byte-identical (`git diff --stat` zero) and that suite was re-run as a regression gate. SteamLogin's THREE top-level returns collapsed into one `renderWindowBody()` inside a SINGLE `<Dialog>`; single-mount is load-bearing — `Dialog` owns its own `open` state and a 500ms entrance transition plus a `gamepadAction` focus hack, so a per-`step` dialog would replay the animation on every step transition. New comment-stripped source gate `steamLoginWindowChrome.test.ts`, all three assertions RED-proven by mutation and cleanly reverted; the frontend jest project is `testEnvironment: 'node'`, so NO DOM assertion is possible and the gate pins source shape only. No new i18n keys — the header reuses the existing `Sign in to Steam` literal from the `<h1>` it replaces, so the localisation gate stays neutral. `pnpm codecheck` clean, eslint 0 errors, 42/42 green. Commits `fdc362444`, `7eb8264df`. **Task 3 is a BLOCKING human-verify gate and was NOT run** — nothing recorded here confirms the window actually renders centred or backdropped, and no source gate can see it; verbatim steps are in SUMMARY.md under "OUTSTANDING: Task 3". Scoped out, pre-existing at HEAD: `.tabsWrapper` in `SteamLogin/index.tsx` has no reachable CSS rule (styled only under `WineManager/index.css:57`). | 2026-08-20 | [260820-kq0-steam-login-should-have-the-same-windowi](.planning/quick/260820-kq0-steam-login-should-have-the-same-windowi/) |
