...
last_updated: "2026-08-21T06:20:00.000Z"
last_activity: "2026-08-22 (NZST) -- Quick task 260822-eib extended the Humble login-chrome CSS to also hide `.simple-navbar`, leaving the login form as the only page content. SUPERSEDES di1's 'keep the navbar as an authenticity cue' reasoning: the navbar is Humble BRANDING, not an ORIGIN indicator, and every real origin cue (macOS sheet origin banner, Windows/Linux OS title, GameLib's own labelled Electron dialog) survives. Not a one-line change -- `simple-navbar` sat in the gates' FORBIDDEN lists and had to invert to REQUIRED. The two lists are deliberately ASYMMETRIC: Rust drops to 4, jest to 7, because `js-view-body`/`js-login-form` must stay -- they are what keeps Humble's React-rendered SIGN-UP link from being hidden. RED proven by real observation (production changed first, tests untouched, three verbatim failures captured), not a synthetic. Re-verified by the orchestrator: literals byte-identical, 199 jest, 8 cargo, tsc clean, i18n still at exactly 1 pre-existing failure. LIVE VISUAL CHECK STILL OWED for both di1 and eib. Commits 2c706aaa9, 8fdf52d8c."
progress:
