...
progress:
  ...
  # 2026-08-22 (phase 34.12 PLANNED): `gsd-sdk query state.planned-phase` corrupted this file
  # AGAIN -- it deleted 417 lines of this block's provenance comments, truncated stopped_at
  # mid-sentence, invented `status: executing` when nothing is executing, wrote nonsense
  # numbers (370/353/71), and -- NEW, and the nastiest variant seen so far -- string-replaced
  # inside an UNRELATED historical table cell ~line 5563, rewriting phase 22's recorded
  # `**Status:** DUD PARKED -- superseded by Phase 24` into `**Status:** Ready to execute`.
  # A closed phase's status was silently rewritten inside prose, far from any field the verb
  # claims to touch. Reverted from a pre-call snapshot; this delta hand-applied instead.
  # total_plans +7 (34.12-01..07) at PLANNED time, matching the planned-time-bump convention
  # documented above. 359 -> 366. completed_plans UNMOVED at 350: planning executes nothing.
