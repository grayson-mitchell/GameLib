...
last_activity: "2026-08-21 (NZST) -- Quick task 260821-ooq ported two upstream Heroic gamepad defects. (1) Console Mode's getActionButtonLabel/getBackButtonLabel branched only on layout.startsWith('ps'), so the 'nintendo' layout that detectControllerLayout() already returns fell through to the Xbox default; Chromium's standard mapping is POSITION-based, so on a Switch pad buttons[0] (mainAction) sits under the physical B cap and buttons[1] (back) under the physical A -- Nintendo users were told to press exactly the wrong button. Labels now return 'B'/'A' for that layout, with button-index-to-action mapping deliberately UNCHANGED (checkStandard and all gamepad_layouts/ files byte-identical). (2) removegamepad() compared controllers.findIndex(...), an ARRAY POSITION, against currentController, a GAMEPAD INDEX -- so disconnecting the active pad usually never fired emitControllerEvent(-1) while a still-connected non-active pad could fire a spurious reset; now wasTracked && gamepad.index === currentController. All four defect gates RED-proven against pre-fix source. New suites 8/8, codecheck and eslint clean. Commits c60eb9776, a1eddb5c3, a4d34a2ea."
progress:
