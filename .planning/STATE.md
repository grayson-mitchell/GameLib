...
last_activity: "2026-08-22 (NZST) -- Quick task 260822-di1 hid Humble's marketing footer (`footer.site-footer`) on BOTH login surfaces via one shared CSS constant: an UNGATED Rust `login_chrome_css_script()` initialization_script on the Tauri child window (its two neighbours are macOS-gated because they substitute for sheet chrome; this substitutes for nothing platform-specific) and a `dom-ready` -> `insertCSS` wiring on the Electron `<webview>`, re-applied per navigation without an idempotence guard because Electron drops inserted CSS on navigation. Preceded by an answered question: Humble has NO API and NO OAuth, and the unofficial `/processlogin` needs a solved reCAPTCHA, so styling the webview -- not replacing it -- is the only route to a cleaner login UI. Hostname gate runs before the idempotence flag so non-Humble hosts keep zero trace; suffix-anchored, RED-proven against the `humblebundle.com.evil.example` look-alike. `#flash`, `#page-top-messages`, `.grayout`, the navbar and all cookie-consent UI are pinned untouched. 198 jest tests, 8 cargo tests, tsc + eslint clean, ALL re-run by the orchestrator. LIVE VISUAL CHECK STILL OWED. Commits 64f06cdc6, de0c7c1a7, dfceb5454."
progress:
