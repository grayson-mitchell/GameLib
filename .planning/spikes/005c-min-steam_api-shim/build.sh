#!/bin/bash
# Build the minimal replacement steam_api.dll shim + a game-like harness (both PE32 i386).
...
cd "$(dirname "$0")"
ZIG="${ZIG:-zig}"
"$ZIG" cc -target x86-windows-gnu -O2 -shared -o steam_api.dll steam_api_shim.c steam_api.def -lws2_32
