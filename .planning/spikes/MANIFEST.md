...
| 024 | epic-store-in-embedded-child-webview | standard | Given a Tauri-managed child webview (`Window::add_child`, spike 016's harness) pointed at `store.epicgames.com`, when the page loads with the injected `window.isTauri`/`__TAURI_INTERNALS__`/`window.ipc`/`__TAURI_PLUGIN_*` globals present (the confirmed, root-caused Talon fingerprint from 2026-08-03), then observe whether Talon blocks STORE browsing the way it blocks the `/id/api/email/exists` LOGIN endpoint, or whether the block is login-scoped only | ✅ VALIDATED 2026-09-05 — **Epic's store renders fine in a Tauri-managed child webview WITH the full fingerprint present** (`isTauri`, `__TAURI__`, `__TAURI_INTERNALS__`, `ipc`, `__TAURI_IIFE__` all read from inside the loaded page), so the Talon block is **login-endpoint-scoped, not store-scoped**. Epic arm: `href=https://store.epicgames.com/?lang=en-US`, real title, `bodyLen=89181`, live sale copy, header painted (`shot-epic-store.png`); no challenge redirect and no `enable javascript and cookies` string. Steam positive control through the SAME embed minutes later also rendered, so a broken harness cannot explain it. Fresh container (`com.gamelib.spike024`) = first-visit case; UA shape matches production's `STORE_EMBED_USER_AGENT`. ⇒ the raw zero-injection `WKWebView` fallback is NOT needed for store browsing, and `/store/epic` becomes a small follow-up — still NOT in Phase 40 (D-05/D-06 unchanged; this spike blocked nothing). NOT proven: product/search/cart pages, anything behind sign-in (still the known-blocked surface, deliberately untouched per D-07), full-viewport layout (embed was 986×117 logical at capture), n=1 on one IP. Bounded probe scope only (D-07): the confirmed 403 is a login-endpoint result, not a store-page result — this spike answers the store-page question, nothing broader. Harness: `.claude/skills/spike-findings-gamelib/sources/016-embedded-child-webview-basic/app/` (reuse spike 016's app, repoint at `store.epicgames.com`). If Epic turns out browsable, `/store/epic` becomes a small follow-up; if not, the only remaining option is a raw zero-injection `WKWebView` subview (its own phase of work — no `WKUIDelegate`, dead Cmd+V, no inspector, `get_webview_window` lookups break). | tauri, webview, epic, talon, anti-bot, embed, store-browser, validated |
...
- **Open before shipping:** input/scroll feel (needs a human on the interactive harness),
  ...
  Phase 40 Plan 10); Epic's anti-bot posture answered by spike 024, RUN 2026-09-05: the store browses fine in a Tauri-managed child webview WITH the fingerprint present, so the Talon block is login-scoped (D-05's scope-out still stands — it was a scheduling decision, not a capability one).
...
- **Inline Password AutoFill and save-password prompts do NOT exist in either login surface**
  (wry WebviewWindow AND pristine WKWebView; loopback-HTTP and real-HTTPS) on macOS 26 —
  live-confirmed; matches Apple's browser-only policy; no public entitlement changes it. *(020)*
- **The system right-click → AutoFill → Passwords panel IS a working Keychain channel in both
  surfaces** — fills credentials even on loopback HTTP; GameLib never touches the secret. UX
  caveats (both surfaces, so system behavior not a harness bug): no site auto-match, panel
  search box rejects typed input with a beep, manual scroll, click-per-field fill. *(020)*
- **Cmd+V paste from the Passwords app works in both surfaces**; the pristine window needs the
