...
// Debug/steam-install-slow-start (cycle 13): mirrors decompress.ts's
// ChunkDecodeError -- tags every decode-stage throw with a short `.code` so
// the `.code`-threading-across-the-real-worker-boundary fix (decompressPool
// .ts's `wireWorker` message handler) has something genuine to exercise
// against an ACTUAL OS thread, not just the in-process handleDecodeMessage
// unit tests.
...
function decompressChunk(buf) {
  ...
  if (magic === 'VZ') {
    if (buf.subarray(-2).toString('latin1') !== 'zv') {
      throw decodeError('bad_footer_magic', 'VZ chunk: bad footer magic')
    }
  ...
  if (magic === 'PK') {
    ...
    return Promise.resolve(
      zlib.inflateRawSync(buf.subarray(30 + nameLen + extraLen))
    )
  }
