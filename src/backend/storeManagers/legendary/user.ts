...
export class LegendaryUser {
  ...
  public static async logout() {
    ...
    // (T-34.5-20): the app-wide cookie jar shares Humble/GOG/Amazon cookies
    // under Tauri (inherited T-34.4.1-47).
    //
    ...
    // cookie-only step above left this branch covering only 1 of the wipe
    // categories the original five-step Electron implementation covered,
    // the exact same shape as humble/user.ts's disconnect() before this same
    // plan fixed it. A second, INDEPENDENT 'clearEpicStorage' step now calls
    // Plan 15's `seam.clearStorage(EPIC_LOGIN_ORIGIN, ...)`, clearing
    // localStorage, sessionStorage, IndexedDB, Cache Storage and
    // service-worker registrations for Epic's own origin only (same-origin
    // policy scopes it structurally, mirroring `clearCookies`'s
    // domain-suffix filter). This closes the `storage`/`cache` categories.
    // It is a SEPARATE wipeSteps entry, not folded into the cookie step, so
    // the guarded loop below keeps treating the two independently. The
    // window this capability opens is opened AND closed entirely inside the
    // Rust arm itself (`humble_login_clear_storage`, Plan 15) — this call
    // site never holds a window handle of its own to leak.
    //
    ...
    // injected JS. This remains a DECLARED, standing residual limitation of
    // this single wipe path (T-34.4.1-73, STRIDE Repudiation, accepted)
    // rather than a silent drop: the residual risk is a cached HTTP auth
    // credential or a DNS cache entry surviving a logout, and NEITHER
    // carries an Epic session — the actual harm (auto-signed-back-in
    // re-login) is fully closed by the cookie + storage steps above. This
    ...
    //     (The five-step legacy session.fromPartition path this paragraph
    //     used to also weigh against was removed by Phase 39 Plan 04 — this
    //     function now has a single wipe path.) Promoting either without a
