...
export default class GOGLibraryManager implements LibraryManager {
  ...
  async getBuilds(appName: string, platform: string, access_token?: string) {
    const url = new URL(
   ...
   */
  async gogToUnifiedInfo(info: GamesDBData | undefined): Promise<GameInfo> {
    if (
      !info ||
   ...
   */
  async createReqsArray(appName: string, os: 'windows' | 'linux' | 'osx') {
    if (!isOnline()) {
      return []
    }
